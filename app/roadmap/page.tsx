...
export default function RoadmapPage() {
  ...
  // KEY: fetch all lessons ONCE, group by lesson.module string into a map
  // { "JavaScript Fundamentals": [...5 lessons], "React Fundamentals": [...3 lessons], ... }
    ...
    const fetchAll = async () => {
      try {
        ...
        if (lessonsRes.ok) {
          ...
          const lessons: Lesson[] = d.lessons || [];

          // Group lessons by their `module` field value
          // This is the fix — each module accordion only sees its own lessons
          const grouped: Record<string, Lesson[]> = {};
          for (const lesson of lessons) {
            const key = lesson.module; // exact string, e.g. "JavaScript Fundamentals"
            if (!grouped[key]) grouped[key] = [];
            grouped[key].push(lesson);
          }

          // Sort each group by the `order` field
  ...
  const modulesWithProgress = modules.map((mod) => {
    ...
    let status: string;
    if (!allLessonsLoaded)              status = mod.defaultStatus;
    else if (done >= total && done > 0) status = 'completed';
    else if (done > 0)                  status = 'progress';
    else                                status = mod.defaultStatus;
                        ...
                        {isExpanded && (
                          ...
                          >
                            <div className="bg-[#0b1828] border border-blue-500/60 border-t-0 rounded-b-2xl px-5 py-5">
                              {!allLessonsLoaded ? (
                                <div className="flex flex-col gap-3">
                                  {Array.from({ length: m.total }).map((_, i) => <LessonCardSkeleton key={i}/>)}
                                </div>
                              ) : m.lessons.length > 0 ? (
