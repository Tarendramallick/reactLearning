...
export default function TrackerPage() {
  ...
  const completedModules = Math.ceil(completedLessons / 6) // Approximate based on lessons
  const overallProgress = totalLessons > 0 ? Math.round((completedLessons / totalLessons) * 100) : 0
  const today = new Date().getDate()

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white font-sans" style={{ fontFamily: "'DM Sans', 'Inter', sans-serif" }}>
      <Navbar />

      {/* ── Custom Page Nav ── */}
      <header className="sticky top-[65px] z-30 bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900/90 backdrop-blur-md border-b border-purple-700/30 px-6 py-4 flex items-center justify-between">
      ...
      <section className="px-6 pt-6 pb-2">
        <div className="grid grid-cols-5 gap-4">
          ...
          <StatCard
            ...
            sub={`${Math.round((completedLessons / totalLessons) * 100)}%`}
          ...
          <StatCard
            ...
            sub={`${Math.round((completedModules / totalModules) * 100)}%`}
